...
//   include 'abcd.php';
  ...
  if(isset($_POST['search']) or isset($_SESSION['search']))
  {
	...
	$start = ($page - 1) * $limit;
      echo $start;
      $result=$conn->query("Select * from resources where filename LIKE '%$string%' and category='workshops' and uploader='$tname' LIMIT $start, $limit");
      $records=$result->fetch_all(MYSQLI_ASSOC);
      
      $result1 = mysqli_query($conn,"SELECT count(id) AS id FROM resources where filename LIKE '%$string%' and category='workshops' and uploader='$tname'  ");
  ...
  $result1 = $conn->query("SELECT count(id) AS id FROM resources where category='workshops' and uploader='$tname'");
  $page = isset($_GET['page']) ? $_GET['page'] : 1;
	$start = ($page - 1) * $limit;
    $result = $conn->query("SELECT * FROM resources where category='workshops' and uploader='$tname' LIMIT $start, $limit");
	$records = $result->fetch_all(MYSQLI_ASSOC);
  ...
  $sqlq=mysqli_query($conn, "Select max(id) as id from resources where category='workshops' and uploader='$tname'");
...
{
echo "<a href='workshop.php' class='button is-black' style='margin-left:1vw;margin-top:0vh;'>All Uploads</a>";
